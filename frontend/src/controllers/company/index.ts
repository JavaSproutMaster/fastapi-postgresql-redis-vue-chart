...
import { CURRENCY } from '@/store/actions/application';
...
const useCompany = (symbol: string): CompanyController => {
  ...
  const stockPriceInPeriod: StockPrice = reactive(emptyStockPriceInPeriod);
  const ceo = reactive([]);
  ...
  const updateFinancials = () => {
    ...
    const quarterly: FinancialSchema[] = [];
    ...
    financialResponse.financials.forEach((financial) => {
      if (!financial.quarter) {
        actual.push(Object.assign(financial, financialFormulas));
      }
      if (financial.quarter) {
        quarterly.push(Object.assign(financial, financialFormulas));
      }
    });
    ...
    for (let i = estimatedYear + 1; i < estimatedYear + 6; i += 1) {
  ...
  const updatePrimaryData = (): Promise<Company> => new Promise((resolve, reject) => {
    api.get(symbol).then((payload) => {
      ...
      store.commit(CURRENCY, payload.currency);
  ...
  const updateStockPriceInPeriod = (period: string):
    Promise<StockPrice> => new Promise((resolve, reject) => {
    api.stockPriceInPeriod(symbol, period).then((payload) => {
      Object.assign(stockPriceInPeriod, payload);
      resolve(payload);
    }).catch((error) => {
      reject(error);
    });
  ...
  const getRecentNotes = () => api.getRecentNotes(symbol);
  const getNotes = (pageNum: number, sort: number) => api.getNotes(symbol, pageNum, sort);
  const addNote = (note: NoteData) => api.addNote(symbol, note);
  const saveNote = (noteId: number, note: NoteData) => api.saveNote(symbol, noteId, note);
  const updateNote = (noteId: number, note: NoteData) => api.updateNote(symbol, noteId, note);
  const deleteNote = (noteId: number) => api.deleteNote(symbol, noteId);
